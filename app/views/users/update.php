...
<body class="bg-gradient-to-br from-purple-200 via-purple-100 to-purple-300 min-h-screen flex items-center justify-center font-sans text-gray-800">
  ...
  <div class="bg-white bg-opacity-90 backdrop-blur-sm p-8 rounded-2xl shadow-2xl w-full max-w-md animate-fadeIn">
    ...
    <form action="<?=site_url('users/update/'.$user['ID'])?>" method="POST" class="space-y-4">
      ...
      <!-- Username -->
      <div>
        <label class="block text-purple-600 mb-1">Username</label>
        <input type="text" name="username" value="<?= html_escape($user['Username'])?>" required
               class="w-full px-4 py-3 border border-purple-200 bg-purple-50 rounded-xl focus:ring-2 focus:ring-purple-300 focus:outline-none text-gray-800">
      </div>

      <!-- Email -->
      <div>
        <label class="block text-purple-600 mb-1">Email Address</label>
      ...
      <?php if(!empty($logged_in_user) && $logged_in_user['role'] === 'admin'): ?>
        ...
        <div>
          <label class="block text-purple-600 mb-1">Role</label>
        ...
        <div class="relative">
          <label class="block text-purple-600 mb-1">Password</label>
